...
LiveLogReader::LiveLogReader(std::string file, std::string uri, bool flipColors, CameraType type)
   ...
   uri(uri)
{
    ...
    if(type == CameraType::OpenNI2)
      cam = new OpenNI2Interface(Resolution::getInstance().width(),Resolution::getInstance().height(), 30,  uri.c_str());
...
void LiveLogReader::getNext()
{
    ...
    m_depth = std::shared_ptr<unsigned short>((unsigned short *)&decompressionBufferDepth[0], std::default_delete<unsigned short[]>());
