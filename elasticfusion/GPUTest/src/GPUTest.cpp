    ...
    for(threads = 16; threads <= 512 && !pangolin::ShouldQuit(); threads += 16)
    {
        for(blocks = 16; blocks <= 512 && !pangolin::ShouldQuit(); blocks += 16)
        {
            ...
            for(int i = 0; i < 5 && !pangolin::ShouldQuit(); i++)
            {
                ...
                icpStepMeanTime = (float(count) * icpStepMeanTime + (Stopwatch::getInstance().getTimings().at("icpStep").back() * 10)) / float(count + 1);
                rgbStepMeanTime = (float(count) * rgbStepMeanTime + (Stopwatch::getInstance().getTimings().at("rgbStep").back() * 10)) / float(count + 1);
                rgbResMeanTime = (float(count) * rgbResMeanTime + (Stopwatch::getInstance().getTimings().at("computeRgbResidual").back() * 10)) / float(count + 1);
                so3StepMeanTime = (float(count) * so3StepMeanTime + (Stopwatch::getInstance().getTimings().at("so3Step").back() * 10)) / float(count + 1);
