...
Deformation::Deformation()
 : def(4, &pointPool),
   pointPool(Eigen::aligned_allocator<Eigen::Vector3f>()),   
   originalPointPool(0),
   firstGraphNode(0),
   sampleProgram(loadProgramGeomFromFile("sample.vert", "sample.geom")),
   bufferSize(2048),//(1024), //max nodes basically
   count(0),
   vertices(new Eigen::Vector4f[bufferSize]),
   poseGraphPoints(Eigen::aligned_allocator<std::pair<uint64_t, Eigen::Vector3f> >()),
   graphPosePoints(new std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>>()),   
   lastDeformTime(0)
{
    //x, y, z and init time
...
bool Deformation::constrain(std::vector<Ferns::Frame*> & ferns,
                            ...
                            std::vector<Constraint, Eigen::aligned_allocator<Constraint>> * newRelativeCons)
{
    if(def.isInit())
    {
        ...
        std::cout << "Deform optimisation: Optimised: " << optimised << " meanConsError: " << meanConsError << " error: " << error << std::endl; 
...
void Deformation::sampleGraphModel(const std::pair<GLuint, GLuint> & model, const int & timeIdx, const int & sampleRate)
{
    sampleProgram->Bind();

    sampleProgram->setUniform(Uniform("timeIdx", timeIdx));
    sampleProgram->setUniform(Uniform("sampleRate", sampleRate));
    ...
    for(int i = 0; i < Vertex::MAX_SENSORS; i++)
    {
        ...
        glVertexAttribPointer(2+i, 1, GL_FLOAT, GL_FALSE, Vertex::SIZE, reinterpret_cast<GLvoid*>(timeOffset + sizeof(float) * (i)));
    }
    
    glEnableVertexAttribArray(2 + Vertex::MAX_SENSORS);
    glVertexAttribPointer(2 + Vertex::MAX_SENSORS, 4, GL_FLOAT, GL_FALSE, Vertex::SIZE, reinterpret_cast<GLvoid*>(timeOffset + sizeof(float) * Vertex::MAX_SENSORS));
    ...
    if((int)count > def.k)
    {
        ...
        struct
        {
            inline bool operator()(const Eigen::Vector4f & a, const Eigen::Vector4f & b)const
            {
                return a(3) < b(3);
            }
        }less;

        std::sort(vertices, vertices + count, less);
