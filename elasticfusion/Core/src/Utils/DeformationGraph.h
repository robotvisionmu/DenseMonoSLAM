...
{
    public:
        DeformationGraph(int k, std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>> * sourceVertices);
        virtual ~DeformationGraph();

        void initialiseGraph(std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>> * customGraph,
        ...
        void setPosesSeq(std::vector<unsigned long long int> * poseTimeMap, const std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>> & poses);
    ...
    private:
        ...
        //Graph itself
        std::vector<GraphNode, Eigen::aligned_allocator<GraphNode>> graphNodes;
        std::vector<GraphNode *> graph;

        //Maps vertex indices to neighbours and weights
        std::vector<std::vector<VertexWeightMap> > vertexMap;
        std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>> * sourceVertices;
        ...
        std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>> * graphCloud;
