...
class Stopwatch
{
    public:
        ...
        void addStopwatchTiming(std::string name, unsigned long long int duration)
        {
            if(duration > 0)
            {
                timings[name].push_back((float)(duration) / 1000.0f);
            }
        }
        ...
        const std::map<std::string, std::vector<float> > & getTimings()
        {
            return timings;
        }
        ...
        void printAll()
        {
            for(std::map<std::string, std::vector<float>>::const_iterator it = timings.begin(); it != timings.end(); it++)
            {
                std::cout << it->first << ": " ;
        ...
        void writeToFile(std::vector<std::string> names, std::string file)
        {
            file.append(".csv");
            std::ofstream ofs(file.c_str(), std::ofstream::out | std::ofstream::binary);
            for(auto & name : names)
            {
                
                if(timings.find(name) == timings.end())continue;
                std::stringstream ss;
                ss << name << ",";
                for(int t = 0; (size_t)t < timings[name].size() - 1; t++)
                {
                    ss << timings[name].at(t) << ",";
                }
                ss << timings[name].back();
                ss <<"\n";
                ofs << ss.str();
            }
            ofs.close();
