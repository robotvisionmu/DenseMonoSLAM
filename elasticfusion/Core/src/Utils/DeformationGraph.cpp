...
DeformationGraph::DeformationGraph(int k, std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>> * sourceVertices)
 : k(k),
   initialised(false),
   wRot(1),
   wReg(10),
   ...
   graphCloud(new std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>>()),
   lastPointCount(0),
...
void DeformationGraph::initialiseGraph(std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>> * customGraph,
...
void DeformationGraph::setPosesSeq(std::vector<unsigned long long int> * poseTimeMap, const std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f> > & poses)
{
    poseMap.clear();
