...
void RGBDOdometry::initICP(GPUTexture * filteredDepth, const float depthCutoff, cudaStream_t stream)
{
    cudaArray * textPtr;

    cudaGraphicsMapResources(1, &filteredDepth->cudaRes, stream);
    ...
    cudaGraphicsUnmapResources(1, &filteredDepth->cudaRes, stream);

    for(int i = 1; i < NUM_PYRS; ++i)
    {
        pyrDown(depth_tmp[i - 1], depth_tmp[i], stream);
    }

    for(int i = 0; i < NUM_PYRS; ++i)
    {
        createVMap(intr(i), depth_tmp[i], vmaps_curr_[i], depthCutoff, stream);
        createNMap(vmaps_curr_[i], nmaps_curr_[i], stream);
    }

    cudaDeviceSynchronize();
}

void RGBDOdometry::initICP(GPUTexture * predictedVertices, GPUTexture * predictedNormals, const float depthCutoff, cudaStream_t stream)
{
    cudaArray * textPtr;

    cudaGraphicsMapResources(1, &predictedVertices->cudaRes, stream);
    ...
    cudaGraphicsUnmapResources(1, &predictedVertices->cudaRes, stream);

    cudaGraphicsMapResources(1, &predictedNormals->cudaRes, stream);
    ...
    cudaGraphicsUnmapResources(1, &predictedNormals->cudaRes, stream);

    copyMaps(vmaps_tmp, nmaps_tmp, vmaps_curr_[0], nmaps_curr_[0], stream);

    for(int i = 1; i < NUM_PYRS; ++i)
    {
        resizeVMap(vmaps_curr_[i - 1], vmaps_curr_[i], stream);
        resizeNMap(nmaps_curr_[i - 1], nmaps_curr_[i], stream);
    }
...
void RGBDOdometry::initRGBModel(GPUTexture * rgb, cudaStream_t stream)
{
...
void RGBDOdometry::initRGB(GPUTexture * rgb, cudaStream_t stream)
{
...
void RGBDOdometry::initFirstRGB(GPUTexture * rgb, cudaStream_t s)
{
    cudaArray * textPtr;

    cudaGraphicsMapResources(1, &rgb->cudaRes, s);

    cudaGraphicsSubResourceGetMappedArray(&textPtr, rgb->cudaRes, 0, 0);

    imageBGRToIntensity(textPtr, lastNextImage[0], s);

    cudaGraphicsUnmapResources(1, &rgb->cudaRes, s);

    for(int i = 0; i + 1 < NUM_PYRS; i++)
    {
        pyrDownUcharGauss(lastNextImage[i], lastNextImage[i + 1], s);
    }
}
                                                ...
                                                cudaStream_t stream)
{
    ...
    iterations[0] = interMap ? 50 : fastOdom ? 3 : 10;
    iterations[1] = interMap ? 50 : pyramid ? 5 : 0;
    iterations[2] = interMap ? 50 : pyramid ? 4 : 0;
...
void RGBDOdometry::getICPResidual(Eigen::Vector3f & trans_curr,
                                  ...
                                  int & icpCount)
{
    ...
    mat33 device_Rcurr = rot_curr;
    float3 device_tcurr = *reinterpret_cast<float3*>(trans_curr.data());

    Eigen::Matrix<float, 3, 3, Eigen::RowMajor> Rprev_inv = rot_curr.inverse();
    mat33 device_Rprev_inv = Rprev_inv;
    float3 device_tprev = *reinterpret_cast<float3*>(trans_prev.data()); // I think this should be the previous pose directly

    DeviceArray2D<float>& vmap_curr = vmaps_curr_[0];
    DeviceArray2D<float>& nmap_curr = nmaps_curr_[0];

    DeviceArray2D<float>& vmap_g_prev = vmaps_g_prev_[0];
    DeviceArray2D<float>& nmap_g_prev = nmaps_g_prev_[0];

    icpRes(device_tcurr, device_Rcurr,
           device_tprev, device_Rprev_inv,
           intr(0),
           angleThres_, distThres_,
           vmap_curr, nmap_curr,
           vmap_g_prev, nmap_g_prev,
           icpCount, icpError);

    icpError = sqrt(icpError) / (float)icpCount;
