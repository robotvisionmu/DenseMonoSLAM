...
void main() 
{
    if(colorType0[0] != -1)
    {
        if(cluster)
        {
            vColor0 = cluster_color;
        }
        else if(colorType0[0] == 1)
        {
        ...
        {
            bool seenC0 = vTimes[0][0] != -3;
            bool seenC1 = vTimes[0][1] != -3;
            bool seenC2 = vTimes[0][2] != -3;
            float g = (seenC0 ? 0.0f : 0.0f) + 
                      (seenC1 ? 0.8f : 0.0f) + 
                      (seenC2 ? 0.0f : 0.0f);   
            float r = (seenC0 ? 0.0f : 0.0f) + 
                      (seenC1 ? 0.1f : 0.0f) + 
                      (seenC2 ? 0.8f : 0.0f);
            float b = (seenC0 ? 0.8f : 0.0f) + 
                      (seenC1 ? 0.2f : 0.0f) + 
                      (seenC2 ? 0.0f : 0.0f);
            float total = (seenC0 ? 1 : 0) + (seenC1 ? 1 : 0) + (seenC2 ? 1 : 0);
            vColor0 = (vec3(r / total, g / total, b / total) * abs(dot(vNormRad[0].xyz, vec3(1.0, 1.0, 1.0)))) + vec3(0.1f, 0.1f, 0.1f);
        }
        else
        {
        ...
        if(drawWindow0[0] == 1 && vTime[0] - vTimes[0][vTimeIdx[0]] > timeDelta0[0])
        {
            vColor0 *= 0.25;
            vec3(0,1,0);
        }
