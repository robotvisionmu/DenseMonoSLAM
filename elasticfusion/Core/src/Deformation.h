...
class Deformation
{
    public:
        ...
        void sampleGraphModel(const std::pair<GLuint, GLuint> & model, const int & timeIdx, const int & sampleRate = 5000);
        ...
        bool constrain(std::vector<Ferns::Frame*> & ferns,
                       ...
                       std::vector<Constraint, Eigen::aligned_allocator<Constraint>> * newRelativeCons = 0);
    ...
    private:
        ...
        std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>> pointPool;
        ...
        std::vector<std::pair<uint64_t, Eigen::Vector3f>, Eigen::aligned_allocator<std::pair<uint64_t, Eigen::Vector3f>>> poseGraphPoints;
        std::vector<unsigned long long int> graphPoseTimes;
        std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>> * graphPosePoints;
