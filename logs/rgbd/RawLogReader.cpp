 ...
 : LogReader(file, flipColors)
{
    ...
    decompressionBufferDepth = new Bytef[480 * 640 * 2];
    decompressionBufferImage = new Bytef[480 * 640 * 3];

    m_depth = std::shared_ptr<unsigned short>(new unsigned short[480 * 640 * 2]);
	m_rgb = std::shared_ptr<unsigned char>(new unsigned char[480 * 640 * 3]);
}
...
void RawLogReader::getCore()
{
    ...
    if(depthSize == numPixels * 2)
    {
        ...
        memcpy(m_depth.get(), depthReadBuffer, numPixels * 2);
    }
    else
    {
        ...
        uncompress((Bytef *)m_depth.get(), (unsigned long *)&decompLength, (const Bytef *)depthReadBuffer, depthSize);
    }

    if(imageSize == numPixels * 3)
    {
        // memcpy(&decompressionBufferImage[0], imageReadBuffer, numPixels * 3);
        memcpy(m_rgb.get(), imageReadBuffer, numPixels * 3);
    }
    else if(imageSize > 0)
    {
        // jpeg.readData(imageReadBuffer, imageSize, (unsigned char *)&decompressionBufferImage[0]);
        jpeg.readData(imageReadBuffer, imageSize, (unsigned char *)m_rgb.get());
    }
    else
    {
        // memset(&decompressionBufferImage[0], 0, numPixels * 3);
        memset(m_rgb.get(), 0, numPixels * 3);        
    }

    // m_depth = std::shared_ptr<unsigned short>((unsigned short *)decompressionBufferDepth, std::default_delete<unsigned short[]>());
    // m_rgb = std::shared_ptr<unsigned char>((unsigned char *)&decompressionBufferImage[0], std::default_delete<unsigned char[]>());
